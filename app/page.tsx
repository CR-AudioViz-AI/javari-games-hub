"use client";
// app/page.tsx — Javari Games Hub · CR AudioViz AI · EIN 39-3646201 · May 2026
import { useState } from "react";
const T=[{"i": "\ud83c\udfae", "l": "Game Discovery", "d": "Find your next game", "h": "/discover"}, {"i": "\u2b50", "l": "Reviews", "d": "Read and write reviews", "h": "/reviews"}, {"i": "\ud83c\udfc6", "l": "Top Lists", "d": "Best games by genre", "h": "/top"}, {"i": "\ud83d\udc65", "l": "Community", "d": "Discuss with gamers", "h": "/community"}];
export default function P() {
  ...
  async function go() { if(!i.trim())return; setL(true);setO("");
    try { const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:i}],stream:false,systemOverride:"You are a gaming expert and critic."})});
  ...
  return (<div style={{minHeight:"100vh",background:"#040912",color:"#e2e8f0",fontFamily:"system-ui"}}>
    <nav style={{background:"#1E3A5F",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>🎮</span><span style={{fontWeight:800,color:"#00B4D8",fontSize:15}}>Javari Games Hub</span></div>
    ...
    <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"48px 24px 40px",textAlign:"center"}}>
      <h1 style={{fontSize:"clamp(22px,4vw,42px)",fontWeight:900,color:"#fff",margin:"0 0 10px",lineHeight:1.05}}>Javari<br/><span style={{color:"#00B4D8"}}>Games Hub</span></h1>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:15,margin:0}}>Discover, review, and discuss games across all platforms.</p>
    </section>
    <section style={{maxWidth:700,margin:"0 auto",padding:"24px 20px 0"}}>
      <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.12)",borderRadius:14,padding:"18px 22px"}}>
        <div style={{display:"flex",gap:8}}><input value={i} onChange={e=>setI(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Recommend games similar to: Elden Ring with a preference for dark fantasy" style={{flex:1,background:"#172D48",border:"1px solid rgba(0,180,216,0.15)",borderRadius:8,padding:"10px 12px",color:"#e2e8f0",fontSize:13,outline:"none",fontFamily:"system-ui"}}/>
